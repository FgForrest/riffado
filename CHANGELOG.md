...
### Added
- Organization scope for self-hosted instances. Setting `ORG_ACCOUNT_EMAIL` and `ORG_ACCOUNT_PASSWORD` creates one organization account and gives every user a shared Organization folder tree next to Private. Owners share a recording by filing it there; nothing is copied. Everyone can open shared recordings, organize the tree, and transcribe or summarize their Organization view, which runs on the clicker's own provider and keys with the organization's templates and never touches the owner's private transcript or summary. Speakers named in shared recordings become Organization people, curated by the organization account and visible in everyone's People list. Withdrawing or deleting a recording removes its Organization view for everyone. Changes appear in other open tabs without reloading. See the Organization page of the self-hosting docs.
...
### Changed
- Migration `0055_shared_org_scope` adds `users.role` (with at most one `org` account), `recording_folders.version` and `created_by_user_id`, `produced_by_user_id` on `transcriptions` and `ai_enhancements`, `people.created_by_user_id`, `recordings.unshared_at`, and the `person_notes` table. Purely additive. On first start, self-host instances retire each user's never-shared per-user Public folder: empty ones are deleted, non-empty ones move under Private as "Former Public".
