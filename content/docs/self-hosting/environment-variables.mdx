...
| `SELF_HOST_MODE`                    | `shared`                           | `shared` allows the Organization scope; `local` keeps every account strictly private.                               |
| `ORG_ACCOUNT_EMAIL`                 | unset                              | With `ORG_ACCOUNT_PASSWORD`, enables the Organization scope: a shared folder tree, and one organization account that curates it. Must not be an existing user's email. |
| `ORG_ACCOUNT_PASSWORD`              | unset                              | Password of the organization account (12+ characters). Re-applied on every start.                                   |
| `ORG_ACCOUNT_NAME`                  | `Organization`                     | Display name of the organization account.                                                                           |
...
| `BACKUP_STORAGE_PATH`   | —           | Filesystem path for backup archives. Unset, they go beside the recordings. |

See [S3-compatible storage](./storage-s3) for per-provider
config templates.

`BACKUP_STORAGE_PATH` is worth setting if you use **Settings →
Export/Backup → Backup frequency**. Without it, every scheduled archive
is written into the same place as the audio it contains — so the folder
roughly doubles in size, and the backup shares a disk with the original.
Point it at separate storage (an external disk, a NAS mount) and
archives go there instead, whatever backend the recordings use. It is
always a local filesystem path, including on S3-backed instances: a
backup you cannot read without the app running is not much of a backup.

## Google integration (Drive export)

Self-host only; ignored when `IS_HOSTED=true`. With the first four set,
users can connect a Google account and export folders to Google Drive.
See [Google Drive export](./google-drive) for the Google Cloud setup.

| Variable                      | Default | Effect                                                                                      |
| ----------------------------- | ------- | ------------------------------------------------------------------------------------------- |
| `GOOGLE_CLIENT_ID`            | —       | OAuth client id ("Web application" client).                                                 |
| `GOOGLE_CLIENT_SECRET`        | —       | OAuth client secret.                                                                        |
| `GOOGLE_PICKER_API_KEY`       | —       | Browser API key for the Google Picker. Restrict it to the Picker API and your `APP_URL`.    |
| `GOOGLE_CLOUD_PROJECT_NUMBER` | —       | Numeric project number of the Google Cloud project (the Picker's app id).                  |
| `GOOGLE_WORKSPACE_DOMAINS`    | —       | Comma-separated Workspace domains allowed to connect. Unset, any account the consent screen admits may. |
