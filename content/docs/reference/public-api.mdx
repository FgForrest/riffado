...
`audio_reaped` is `true` when the owner's retention policy has deleted
the audio. The recording and its metadata remain; `links.audio` answers
`410 Gone`. Check this flag rather than treating a 410 as an error.
